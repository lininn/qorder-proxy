...
async function startCommand(cliFlags) {
  ...
  process.env.CLI_BACKEND = config.backend || 'cn';
  const backend = getCliBackend();
  const hasOAuthLogin = hasStoredCredentials(backend);

  if (!config.token && !hasOAuthLogin) {
    console.log('⚠️  未配置认证 Token！');
    console.log('   请先运行 "qorder-proxy --web" 配置 Token，使用 --token 参数，');
    console.log(`   或运行 "${backend.command} login" 通过浏览器登录。`);
  ...
  if (!config.token && hasOAuthLogin) {
    console.log(`   Auth: browser login (${backend.command} login)`);
  }
